...
const ModelComparisonRadar = dynamic(() => import("@/components/model-comparison-radar").then(m => m.ModelComparisonRadar), {
  ssr: false,
  loading: () => <Typography.Text type="secondary">加载中...</Typography.Text>
});

const RiskAssessmentRadar = dynamic(() => import("@/components/model-comparison-radar").then(m => m.RiskAssessmentRadar), {
  ssr: false,
  loading: () => <Typography.Text type="secondary">加载中...</Typography.Text>
});
...
export function DashboardClient({
  ...
  function handleAdvance(record: WorkItemView, label: string) {
    ...
    if (record.riskLevel === "高") {
      modal.confirm({
        ...
        content: `当前任务为高优先级咨询：${record.title}，推进后将写入新的服务留痕。`,
        okText: "确认推进",
        cancelText: "取消",
  ...
  function handleDelete(record: WorkItemView, title?: string, text?: string) {
    modal.confirm({
      title: title ?? `确认删除该${appMeta.sourceObjectName}？`,
      icon: <ExclamationCircleOutlined />,
      content: text ?? `删除后将同步移除关联服务留痕：${record.title}`,
      okText: "确认删除",
    ...
    return (
      <Space direction="vertical" size={16} className="full-width">
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} md={8}>
            <Card>
              <Statistic title="高优先级待办" value={riskCount} suffix="条" valueStyle={{ color: "#d93026" }} />
            </Card>
          </Col>
          <Col xs={24} md={8}>
            <Card>
          ...
          <Col xs={24} lg={9}>
            <Card title="服务留痕">
              {snapshot.events.length === 0 ? (
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无服务留痕" />
              ) : (
                <Timeline
    ...
    return (
      <>
        <Space direction="vertical" size={16} className="full-width">
          ...
          {routeKey === "qa-operations" && (
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={12}>
                <ModelComparisonRadar snapshot={snapshot} />
              </Col>
              <Col xs={24} lg={12}>
                <RiskAssessmentRadar snapshot={snapshot} />
              </Col>
            </Row>
          )}
        </Space>
