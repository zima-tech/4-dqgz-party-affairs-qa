...
async function main() {
  ...
  await prisma.modelEvent.deleteMany();
  await prisma.aiDraft.deleteMany();
  await prisma.integrationLog.deleteMany();
  await prisma.insight.deleteMany();
  await prisma.investModel.deleteMany();
      ...
      data: [
        {
          ...
          action: "初始化导入",
          actor: item.owner,
          content: `来自${item.sourceType}《${item.sourceTitle}》：${record.modelType}已创建，领域${record.industry}，范围${record.region}，等待质检发布。`,
        },
        {
          modelId: item.id,
          sourceType: item.sourceType,
          ...
          content: `来自${item.sourceType}《${item.sourceTitle}》：已完成初步政策核验，服务优先级${item.riskLevel}，建议补充岗位化学习推荐。`,
        },
      ],
    });
