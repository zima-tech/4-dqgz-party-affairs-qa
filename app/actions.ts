...
export async function createWorkItem(input: WorkItemInput) {
  ...
  const item = await prisma.investModel.create({
    data: {
      code,
      title: input.title,
      modelType: input.category,
      industry: "",
      region: "",
  ...
  await prisma.modelEvent.create({
    data: {
      ...
      action: "创建知识条目",
      actor: input.owner,
      content: `来自${item.sourceType}《${item.sourceTitle}》：${input.category}已创建，结构化摘要和责任岗位已同步。`,
    },
  });
  ...
  await recordAuditLog({
    ...
    actor: input.owner,
    summary: `${item.modelType}已创建并同步责任人。`,
  });
...
export async function advanceWorkItem(id: string) {
  const item = await prisma.investModel.findUniqueOrThrow({ where: { id } });
  const status = nextStatus(item.status);
  await prisma.investModel.update({ where: { id }, data: { status } });
  await prisma.modelEvent.create({
    data: {
      modelId: id,
      sourceType: item.sourceType,
      ...
      action: "服务推进",
      actor: item.owner,
      content: `来自${item.sourceType}《${item.sourceTitle}》：处理状态由${item.status}推进至${status}。`,
    },
  });
  ...
  await recordAuditLog({
    ...
    targetName: item.title,
    actor: item.owner,
    summary: `${item.modelType}处理状态由${item.status}推进至${status}。`,
  });
...
export async function deleteWorkItem(id: string) {
  const item = await prisma.investModel.findUniqueOrThrow({ where: { id } });
  await prisma.investModel.delete({ where: { id } });
  await recordAuditLog({
    module: "问答服务运营",
    action: "删除条目",
    targetType: appMeta.sourceObjectName,
    targetName: item.title,
    actor: item.owner,
    summary: `${item.modelType}记录已删除，关联服务留痕同步移除。`,
  });
  revalidatePath("/");
  return getDashboardSnapshot();
}
...
export async function generateAiDraft(input: GenerateAiDraftInput) {
  ...
  try {
    const item = await prisma.investModel.findUnique({
    ...
    const existingTurns: DraftTurn[] = input.conversationId
    ...
    await recordAuditLog({
      ...
      targetName: item.title,
      actor: appMeta.shortName,
      summary: response.source === "glm" ? "已完成智能问答并保存会话结果。" : "已基于当前业务数据形成答复初稿。",
    });
